...
>
    <section class="relative overflow-hidden pt-2 sm:pt-3 pb-4 sm:pb-5" aria-labelledby="category-heading">
        <div class="container mx-auto px-4 sm:px-6 frontend-container">
            <div class="bg-white dark:bg-zinc-900 rounded-[2rem] p-6 sm:p-10 lg:p-14 pt-[5.75rem] sm:pt-[6.5rem] lg:pt-[6.75rem] ring-1 ring-zinc-900/[0.04] dark:ring-white/[0.06] shadow-[0_1px_3px_rgb(16_24_40_/_0.03),0_12px_32px_-16px_rgb(16_24_40_/_0.08)]">
                ...
                {{-- Header --}}
                <div class="flex flex-col sm:flex-row sm:items-center gap-6 mb-10 sm:mb-12">
                    @if($category->image)
                        <div class="w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0 overflow-hidden rounded-3xl bg-zinc-50 dark:bg-zinc-800/60 ring-1 ring-zinc-900/[0.04] dark:ring-white/[0.06]">
                            <img
                    ...
                    <div class="max-w-2xl">
                        <div class="flex flex-wrap items-center gap-2 mb-4">
                            ...
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-50 dark:bg-zinc-800/60 text-zinc-600 dark:text-zinc-400 text-xs font-semibold uppercase tracking-widest ring-1 ring-zinc-900/[0.06] dark:ring-white/[0.08]">
                                {{ trans_choice(':count product|:count products', $this->totalProductCount, ['count' => $this->totalProductCount]) }}
                            </span>
                        </div>
                        <h1 id="category-heading" class="font-display text-3xl sm:text-4xl lg:text-[2.75rem] font-bold leading-tight text-zinc-900 dark:text-white tracking-tight text-balance">
                            {{ $category->name }}
                        </h1>
                ...
                @elseif($this->siblingCategories->count() > 0)
                    {{-- Sibling category chips (leaf-category fallback) --}}
                    <div class="mb-8">
                        <span class="block text-xs font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-2">{{ __('Other categories') }}</span>
                ...
                <div class="flex flex-col lg:flex-row lg:items-center gap-3 mb-6">
                    <div class="relative flex-1 min-w-0">
                        ...
                        <label for="category-search" class="sr-only">{{ __('Search in :category', ['category' => $category->name]) }}</label>
                        <input
                            id="category-search"
                            type="search"
                            wire:model.live.debounce.400ms="search"
                            placeholder="{{ __('Search in :category...', ['category' => $category->name]) }}"
                ...
                <div id="product-results" class="flex flex-wrap items-center justify-between gap-3 mb-4">
                    ...
                    <div class="flex items-center gap-3">
                        {{-- Per-page --}}
                        <div class="relative">
                            <label for="category-per-page" class="sr-only">{{ __('Products per page') }}</label>
                            <select
                                id="category-per-page"
                ...
                {{-- Active filter chips --}}
                @if($search !== '' || $sort !== 'featured' || $minPrice !== '' || $maxPrice !== '' || $inStockOnly)
                ...
                <div class="lg:grid lg:grid-cols-[260px_1fr] lg:gap-8 xl:gap-10">
                    ...
                    <aside class="hidden lg:block">
                        <div class="lg:sticky lg:top-28 bg-zinc-50 dark:bg-zinc-800/60 rounded-3xl ring-1 ring-zinc-900/[0.04] dark:ring-white/[0.06] p-6">
                            ...
                            @include('components.public.category-filters')
                        </div>
                    </aside>
