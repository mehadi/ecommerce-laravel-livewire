...
<div class="space-y-4">
    <div
        ...
        @click="browse"
        class="flex cursor-pointer flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed bg-zinc-50/70 p-8 text-center transition-colors dark:bg-zinc-900/60 {{ $baseClass }} {{ $hoverClass }}"
        :class="isDragging ? '{{ $highlightClass }}' : ''"
    >
        <div class="flex flex-col items-center gap-3">
            <div class="flex size-12 items-center justify-center rounded-full {{ $iconClasses }}">
            ...
            <flux:button type="button" size="xs" variant="outline" @click.stop="browse">
                {{ $buttonText }}
            </flux:button>

            @if($helperText)
                <p class="text-xs text-zinc-400 dark:text-zinc-500">{{ $helperText }}</p>
            @endif
        </div>
            ...
            <div class="relative w-full max-w-sm overflow-hidden rounded-xl border border-zinc-200 shadow-sm dark:border-zinc-700">
                ...
                @if($removeMethod)
                    <button
                        ...
                        class="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-zinc-900/75 px-3 py-1 text-xs font-medium text-white backdrop-blur transition hover:bg-red-600/90"
                    >
                ...
                <div class="group relative overflow-hidden rounded-xl border border-zinc-200 shadow-sm transition hover:shadow-md dark:border-zinc-700" wire:key="{{ $wireModel }}-preview-{{ $index }}">
                    ...
                    @if($removeMethod)
                        <button
                            ...
                            class="absolute right-3 top-3 inline-flex items-center justify-center rounded-full bg-zinc-900/70 p-1.5 text-white backdrop-blur transition hover:bg-red-600/90"
                        >
    ...
    <div wire:loading wire:target="{{ $wireModel }}" class="text-xs font-medium {{ $loadingClass }}">
        {{ $loadingText }}
    </div>
</div>
