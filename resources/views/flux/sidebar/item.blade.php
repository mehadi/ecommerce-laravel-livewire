...
$iconClasses = Flux::classes('size-4 transition-colors duration-150')
...
$classes = Flux::classes()
    ->add('h-9 in-data-flux-sidebar-on-mobile:h-11 relative flex items-center gap-3 rounded-lg')
    ...
    ->add('text-zinc-500 dark:text-white/80 transition-colors duration-150')
    ->add(match ($accent) {
        true => [
            // Modern "current" state: soft emerald tint + a slim brand-colored
            // indicator bar on the leading edge, instead of a plain bordered box.
            'data-current:text-emerald-700 dark:data-current:text-emerald-400',
            'data-current:bg-emerald-500/10 hover:data-current:bg-emerald-500/10 dark:data-current:bg-emerald-400/10',
            'data-current:font-semibold',
            'data-current:before:absolute data-current:before:start-0 data-current:before:inset-y-1.5 data-current:before:w-[3px] data-current:before:rounded-s-md data-current:before:bg-emerald-500 data-current:before:content-[\'\'] in-data-flux-sidebar-collapsed-desktop:data-current:before:hidden',
