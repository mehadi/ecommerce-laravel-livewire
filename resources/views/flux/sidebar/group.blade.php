...
<?php if ($expandable && $heading): ?>
    <?php if ($icon): ?>
        <ui-disclosure {{ $attributes->class('group/disclosure in-data-flux-sidebar-collapsed-desktop:hidden') }} @if ($expanded === true) open @endif data-flux-sidebar-group>
            <button type="button" class="border-1 border-transparent w-full h-8 in-data-flux-sidebar-on-mobile:h-11 flex items-center group/disclosure-button my-px rounded-lg transition-colors duration-150 hover:bg-zinc-800/5 dark:hover:bg-white/[7%] text-zinc-400 hover:text-zinc-700 dark:text-zinc-500 dark:hover:text-white">
                <div class="px-3">
                ...
                <span class="flex-1 text-left rtl:text-right text-[11px] font-semibold uppercase tracking-wider leading-none">{{ $heading }}</span>

                <div class="ps-3 pe-2.5">
                    <flux:icon.chevron-right class="size-3! transition-transform duration-200 group-data-open/disclosure-button:rotate-90 rtl:-scale-x-100" />
                </div>
            </button>

            <div class="relative hidden data-open:block ps-7" @if ($expanded === true) data-open @endif>
                <div class="absolute inset-y-[3px] w-px bg-zinc-200/70 dark:bg-white/10 start-0 ms-5"></div>
